...
async function mm(
  ...
  start_best_bid =
    start_best_bid == Number.MIN_SAFE_INTEGER ? min_bid : start_best_bid;
  start_best_ask =
    start_best_ask == Number.MAX_SAFE_INTEGER ? min_ask : start_best_ask;

  //place the orders
  let initial_bid = await market.placeOrder(connection, {
    owner: mm_owner,
    payer: payer_quote,
    side: "buy",
  ...
  let initial_ask = await market.placeOrder(connection, {
    owner: mm_owner,
    payer: payer_base,
    side: "sell",
  ...
  while (true) {
    ...
    let mm_orders = await market.loadOrdersForOwner(
      connection,
      mm_owner.publicKey
    );
    ...
    if (best_bid > mm_bid) {
      ...
      //place new bid
      let up_bid = await market.placeOrder(connection, {
        owner: mm_owner,
        payer: payer_quote,
        side: "buy",
        price: best_bid,
    ...
    if (best_ask < mm_ask) {
      ...
      //place new ask
      let up_ask = await market.placeOrder(connection, {
        owner: mm_owner,
        payer: payer_base,
        side: "sell",
        price: best_ask,
    ...
    for (let order of mm_orders) {
      if (order.side == "buy") {
        mm_bid_size += order.size;
        cur_bid = order.price;
      }
      if (order.side == "sell") {
        mm_ask_size += order.size;
        cur_ask = order.price;
      }
    }
...
function checkMarketState(
  bids_before: Orderbook,
  bids_after: Orderbook,
  asks_before: Orderbook,
  asks_after: Orderbook
) {
  ...
  for (let bid_before of bids_before) {
    ...
    for (let bid_after of bids_after) {
      if (
        bid_before.orderId.eq(bid_after.orderId) &&
  ...
  for (let ask_before of asks_before) {
    ...
    for (let ask_after of asks_after) {
      if (
        ask_before.orderId.eq(ask_after.orderId) &&
