...
import { blob, struct, u32 } from "@solana/buffer-layout";
...
export interface MarketMaker {
  market: Market;
  initialize_positions(): Promise<
    [number, number, OrderState[], OrderState[], number]
  >;
  onBook(
    est_bid: number,
    best_ask: number,
    mm_bids: OrderState[],
    mm_asks: OrderState[]
  ): Promise<[number, number, OrderState[], OrderState[]]>;
  onFill(
    bid_qty_refill: number,
    ask_qty_refill: number,
    mm_bids: OrderState[],
    mm_asks: OrderState[]
  ): Promise<[OrderState[], OrderState[]]>;
}
...
export class SimpleMarketMaker implements MarketMaker {
  ...
  async sendTxn(txn: Transaction, signers: Array<Account>) {
    try {
      ...
      const { value } = await this.connection.confirmTransaction(signature);
      return signature;
    } catch {}
  }

  async initialize_positions(): Promise<
    [number, number, OrderState[], OrderState[], number]
  > {
  ...
  async onFill(
    ...
    if (bid_qty_refill >= ORDER_SIZE || ask_qty_refill >= ORDER_SIZE)
      return [mm_bids, mm_asks]; // Let onBook manage this (Full Fill case)
    ...
    if (bid_qty_refill > 0) {
      ...
      mm_bids.push(
        new OrderState(clientID, "buy", this.bid_pos, bid_qty_refill)
      );
    }
    ...
    if (ask_qty_refill > 0) {
      ...
      mm_asks.push(
        new OrderState(clientID2, "sell", this.ask_pos, ask_qty_refill)
      );
    }
